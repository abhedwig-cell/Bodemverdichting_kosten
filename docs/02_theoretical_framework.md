...
This implies that project outputs may legitimately be:

- separate scenario results;
- a bounded range over qualified scenarios;
- a frequency-weighted expected result, but only when occurrence frequencies are independently qualified;
- or no numerical result when the relevant conditioning set is not sufficiently supported.
